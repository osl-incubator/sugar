# src/sugar/tui/screens/services.py
from textual.app import ComposeResult
from textual.screen import Screen
from textual.widgets import DataTable, Button, Static, Header, Footer, Rule, Label
...
class ServiceScreen(Screen):
    ...
    # Match the same CSS path as profiles screen
        ...
        yield Container(
            Vertical(
                ...
                Horizontal(
                    ...
                    id="profile-actions"  # Reuse the same ID to match CSS
                ),
    ...
    def _create_services_table(self) -> DataTable:
        ...
        table.styles.height = 7  # Reduced to make room for details section
        table.styles.max_height = 7
        
        table.add_columns("Service", "Status", "Ports", "Image", "Container ID")
        
        # This would be populated from actual Sugar config in the future
        table.add_rows([
            ("frontend", "● Running", "3000:3000", "node:16", "abc123def456"),
            ("api", "● Running", "8000:8000", "python:3.10", "def456ghi789"),
            ("db", "● Running", "5432:5432", "postgres:14", "789ghi101112"),
            ("redis", "● Running", "6379:6379", "redis:latest", "jkl131415mno"),
            ("nginx", "● Running", "80:80, 443:443", "nginx:latest", "pqr161718stu"),
    ...
    def on_mount(self) -> None:
        ...
        # Update the details section with the first service's data
        self._update_service_details("frontend", "● Running", "3000:3000", "node:16", "abc123def456")
    
    def _update_service_details(self, name, status, ports, image, container_id):
        ...
        if not self.query("#service-details"):
            details = Vertical(
                ...
                Horizontal(
                    ...
                    Label(container_id, classes="detail-value", id="container-id")
                ),
                id="service-details"
            )
    ...
    def on_data_table_row_selected(self, event):
        ...
        if row_idx is not None:
            ...
            self._update_service_details(row_data[0], row_data[1], row_data[2], row_data[3], row_data[4])
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if button_id == "start-service":
            ...
            try:
                ...
                self.notify(f"Error opening logs: {str(e)}", severity="error")
        elif button_id == "details-service":
            try:
                self.app.push_screen("details")
            except Exception as e:
                self.notify(f"Error opening details: {str(e)}", severity="error")